...
def Paddingtobit(Input):
    ...
    if(len(temp)%512!=0):
        ...
        temp += '1' 
        n = round(len(temp)/512)
        if(n==0):
            temp+=("0"*(((512)-64) - len(temp)))
        else:
            temp+=("0"*(((n * 512)-64) - len(temp)))
...
def Splitter(Msg,n):
    ...
    if(len(Msg)==n):
        ...
        temp = [Msg[(n*(ni-1)) : (n*(ni) -1) ] for ni in range(1,(int)(len(Msg)/n)+1)]
    return temp 
...
def Key_based_md5(Msg,key,salt):
    ...
    for _ in range(100):
        ...
        Hash = A+B+C+D
        I1 = Splitter(Paddingtobit(Stringtobinary(Hash)),512)
    return Hash
...
if __name__=='__main__':
    ...
    Hash2 = Key_based_md5(Msg,"790abe22","89eea11")
